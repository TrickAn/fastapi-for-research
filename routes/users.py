...
@Users.post("/users/", response_model=schemas.users.User)
...
@Users.get("/users/", response_model=list[schemas.users.User])
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.users.get_users(db, skip=skip, limit=limit)
    return users


@Users.get("/users/{user_id}", response_model=schemas.users.User)
